...
const Navbar = () => {
  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 bg-background/80 backdrop-blur-lg">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        {mobileMenuOpen && <div className="md:hidden fixed inset-x-0 top-16 p-4 bg-background/95 backdrop-blur-lg z-50 border-t border-white/10">
            <div className="flex flex-col space-y-4 py-4">
              <Link to="/dashboard" className={`${isActive('/dashboard')} px-4 py-2 font-archivo`} onClick={() => setMobileMenuOpen(false)}>
                Dashboard
              </Link>
              <Link to="/impact-projects" className={`${isActive('/impact-projects')} px-4 py-2 font-archivo`} onClick={() => setMobileMenuOpen(false)}>
                Impact Projects
              </Link>
              <Link to="/blockchain-tracker" className={`${isActive('/blockchain-tracker')} px-4 py-2 font-archivo`} onClick={() => setMobileMenuOpen(false)}>
                Blockchain Tracker
              </Link>
              <Link to="/ai-workforce" className={`${isActive('/ai-workforce')} px-4 py-2 font-archivo`} onClick={() => setMobileMenuOpen(false)}>
                AI Workforce
              </Link>
              <Link to="/agents" className={`${isActive('/agents')} px-4 py-2 font-archivo`} onClick={() => setMobileMenuOpen(false)}>
                AI Agents
              </Link>
